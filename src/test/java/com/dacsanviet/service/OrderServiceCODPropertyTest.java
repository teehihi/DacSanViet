...
public class OrderServiceCODPropertyTest {
	...
	private void setupMocks() {
		...
		orderService = new OrderService(orderRepository, orderItemRepository, cartItemRepository, userRepository,
				addressRepository, productRepository, notificationService, emailService);
	}

	/**
	 * **Feature: cod-payment-workflow, Property 1: COD Order Creation State
	 * Management** **Validates: Requirements 1.2, 1.4, 1.5**
	 * 
	 * For any COD checkout request, creating the order should result in PROCESSING
	 * status, PENDING payment status, and empty user cart
	 */
	@Property(tries = 100)
	void codOrderCreationStateManagement(@ForAll @Positive Long userId, @ForAll @NotBlank String customerName,
			@ForAll @NotBlank String customerPhone, @ForAll @NotBlank String customerEmail,
			@ForAll @NotBlank String shippingAddress, @ForAll @Positive BigDecimal productPrice,
	 ...
	 * 
	 * For any order in SHIPPED status, when user confirms delivery, the system
	 * should update status to DELIVERED, set payment status to COMPLETED, record
	 * confirmation timestamp, and prevent further user status changes
	 */
	@Property(tries = 100)
	void deliveryConfirmationWorkflow(@ForAll @Positive Long orderId, @ForAll @Positive Long userId) {
	 ...
	 * 
	 * For any COD order creation attempt, the system should validate required
	 * customer information, and reject orders with missing data
	 */
	@Property(tries = 100)
	void codOrderValidation(@ForAll @Positive Long userId, @ForAll @Positive BigDecimal productPrice,
	...
	/**
	 * **Feature: cod-payment-workflow, Property 5: Order Status Transition
	 * Validation** **Validates: Requirements 5.2**
	 * 
	 * For any order status change attempt, the system should validate transition
	 * rules and reject invalid transitions
	 */
	@Property(tries = 100)
	void orderStatusTransitionValidation(@ForAll @Positive Long orderId, @ForAll @Positive Long userId) {
		...
		assertThat(exceptionThrown).as("Should have thrown exception for invalid status transition from DELIVERED")
				.isTrue();

		// Test case 3: Invalid transition from PROCESSING to DELIVERED (skipping
		// SHIPPED)
		setupMocks();
		...
		assertThat(skipExceptionThrown)
				.as("Should have thrown exception for invalid status transition from PROCESSING to DELIVERED").isTrue();
	}

	/**
	 * **Feature: cod-payment-workflow, Property 6: Delivery Confirmation
	 * Authorization** **Validates: Requirements 5.3**
	 * 
	 * For any delivery confirmation attempt, the system should validate the order
	 * belongs to the requesting user and reject unauthorized attempts
	 */
	@Property(tries = 100)
	void deliveryConfirmationAuthorization(@ForAll @Positive Long orderId, @ForAll @Positive Long userId,
		...
		assertThat(authExceptionThrown).as("Should have thrown exception for unauthorized delivery confirmation")
				.isTrue();
